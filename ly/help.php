<?php
use ly\lib\DB as DB;
if(!function_exists("json_decode_pro")){
    function json_decode_pro($str,$toArray=false){
        $str = preg_replace("/,[\r\n]*\s*([\]\}])/", "$1", $str);
...
if (!function_exists('json_encode_pro')) {
    function json_encode_pro($array)
    {
        return json_encode($array, JSON_UNESCAPED_UNICODE);
    }
}
