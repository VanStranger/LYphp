...
class LY
{
    ...
    public function getMethodAnnotation($class, $fn, $key)
    {
        ...
        if ($matches) {
            ...
            if(method_exists($ref,"getAttributes")){
                $r=[];
                $spacename = (new \ReflectionClass($class))->getNamespaceName();
                foreach ($ref->getAttributes() as $k => $v) {
                    if($v->getName()===$spacename."\\".$key){
                        $r=$v->getArguments();
                        break;
                    }
                }
    ...
    public function run()
    {
        ...
        if($this->config['PRODUCTION_MODE']){
        ...
        if (is_file($file)) {
            ...
            $res        = null;
            $annotations = $this->getMethodAnnotation($controller, $action, "Before");
            if (count($annotations)) {
                $i=0;
                while(is_null($res) && $i<count($annotations)){
                    $res = $this->execClassMethod($controller, $annotations[$i]);
                    $i++;
                }
            }
            if (is_null($res)) {
                ...
                foreach ($beforeArr as $key => $value) {
                    if (!is_numeric($key)) {
                        ...
                        if (A === $key) {
                            ...
                            $annotations = $this->getMethodAnnotation($controller, $action, "Skip");
                            if (!in_array($key,$annotations)) {
